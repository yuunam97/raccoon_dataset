...
### 1) Load the model; 2) Load the images; 3) Perform Selective Search
...
### Extracting each of proposal ROIs and pre-process them:
...
# classify each of the proposal ROIs using fine-tuned model: PERFORMING INFERENCE
print("[INFO] classifying proposals...")
proba = model.predict(proposals)


### Filtering for all `raccoon` predictions, dropping the `no_raccoons` results:
...
### Viewing the results WITHOUT NMS applied:
...
### Viewing the results WITH NMS applied:

# run non-maxima suppression on the bounding boxes
boxIdxs = non_max_suppression(boxes, proba)

# Print out the indices returned by non_max_suppression

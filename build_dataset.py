...
## Creating empty directories and build a list of all the raccoon images:
...
## Looping over all the `imagePaths` in the dataset:
...
for (i, imagePath) in enumerate(imagePaths):
	...
	# print(soup.prettify().encode('cp1252', errors='replace').decode('cp1252'))
	gtBoxes = []
	...
	# loop over all 'object' elements in the annotation files:
...
## Here we have 1) ground-truth bounding boxes and 2) region proposals generated by Selective Search, 
## we use IoU to determine which regions overlap sufficiently with the ground-truth boxes and which do not:

	# initialize counters used to count the number of positive and negative ROIs saved thus far
	positiveROIs = 0 # --> Saved in config.POSITIVE_PATH
	negativeROIs = 0 # --> Saved in config.NEGATIVE_PATH
	...
	for proposedRect in proposedRects[:config.MAX_PROPOSALS]:
		...
		for gtBox in gtBoxes:
			...
			# check to see if there is not full overlap *and* the IoU is less than 5% *and* we have not hit our negative
			# count limit
			...
			if roi is not None and outputPath is not None:
				# resize the ROI to the input dimensions of the that we'll be fine-tuning, then write the ROI to disk
